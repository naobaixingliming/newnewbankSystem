...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
  ...
  $stateProvider
    ...
    //新增  bu mingyubo on 20171018
    .state('app.access-code', {
        url: '/access-code',
        title: 'Timeline',
        templateUrl: helper.basepath('access-code.html')
    })
    .state('app.userInfo', {
        url: '/userInfo',
        title: '用户信息',
        templateUrl: helper.basepath('_user-information.html'),
        resolve: helper.resolveFor('datatables', 'datatables-pugins','ngDialog')
    })
    .state('app.userInfo-authentice', {
        url: '/userInfo-authentice',
        title: '用户认证信息',
        templateUrl: helper.basepath('_userInfo-authentice.html'),
        resolve: helper.resolveFor('datatables', 'datatables-pugins','ngDialog')
    })
    .state('app.user-feedbackList', {
        url: '/user-feedbackList',
        title: '用户反馈列表',
        templateUrl: helper.basepath('_userInfo-feedbackList.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins','ngDialog')
    })
    .state('app.agent-rank', {
        url: '/agent-rank',
        title: '用户代理等级',
        templateUrl: helper.basepath('_agent-userAgentRank.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins','ngDialog')
    })
    .state('app.agent-list', {
        url: '/agent-list',
        title: '代理商列表',
        templateUrl: helper.basepath('_agent-list.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins','ngDialog')
    })
    .state('app.agent-fundAcount', {
        url: '/agent-fundAcount',
        title: '奖励资金账户',
        templateUrl: helper.basepath('_agent-bonusFundAccount.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins')
    })
    .state('app.agent-awardsGetRecords', {
        url: '/agent-awardsGetRecords',
        title: '奖励获得记录',
        templateUrl: helper.basepath('_agent-awardsGetRecords.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins')
    })
    .state('app.agent-bonusRecord', {
        url: '/agent-bonusRecord',
        title: '奖励打款记录',
        templateUrl: helper.basepath('_agent-BonusRecord.html'),
        resolve: helper.resolveFor('datatables','datatables-pugins')
    })
     .state('app.maintain-tableField', {
        url: '/table-field',
        title: '表字段维护',
    ...
    .state('app.recall-rateStatis', {
        ...
        title: '催回率统计',
        templateUrl: helper.basepath('_j_recallRateStatistics.html'),
        resolve: helper.resolveFor('xeditable')
    })
    .state('app.collecte-orderStatis', {
        url: '/collecte-orderStatis',
        title: '催收订单统计',
        templateUrl: helper.basepath('_j_collecteOrderStatis.html'),
        resolve: helper.resolveFor('xeditable')
    })
    .state('app.daily-statisticsColl', {
        url: '/daily-statisticsColl',
        title: '催收员每日统计',
...
App
  ...
  .constant('APP_REQUIRES', {
    ...
    modules: [
      ...
      {name: 'ngDialog',                  files: [// 'https://cdn.bootcss.com/ng-dialog/1.0.0/js/ngDialog.min.js',
...
/**=========================================================
 * Module: sidebar-menu.js  (模块：sidebar-menu.js)
 * Handle sidebar collapsible elements  (处理侧边栏可折叠元素)
 =========================================================*/
...
App.controller('SidebarController', ['$rootScope', '$scope', '$state', '$http', '$timeout', 'Utils',
  function($rootScope, $scope, $state, $http, $timeout, Utils){
    ...
    // Load menu from json file (从JSON文件加载菜单)
    // ----------------------------------- 
